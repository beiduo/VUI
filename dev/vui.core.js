...
// 原型继承和属性拷贝的混合应用 (先继承对象o，再添加额外的方法和属性)
...
function getStyle(elem, name){
    ...
    /*W3C标准*/
    if (document.defaultView && document.defaultView.getComputesStyle){
 ...
 */
function resetCSS(elem, css_obj){
...
// 事件绑定
VUI.on = function(ele, event, fn){
    var ele = Sizzle(ele);
    for (var i = 0; i < ele.length; i++){
        addEvent(ele[i], event, fn);
    }
}
